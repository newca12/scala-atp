...
class OrderDsl(val input: ParserInput) extends Parser {
  ...
  def items = rule { ws('(') ~ oneOrMore(line_item).separatedBy(", ") ~ ws(')') ~> Items }
  ...
  def security_spec = rule {
    numericLit ~ WhiteSpace ~ ident ~> ((n: Int, s) ⇒ SecuritySpec(n.toInt, s.toString)) ~
      WhiteSpace ~ "shares" ~ WhiteSpace
  }

  def price_spec: Rule1[PriceSpec] = rule {
    "at " ~ optional(min_max) ~> ((m: Option[PriceType]) ⇒ m) ~ numericLit ~
      WhiteSpace ~> (PriceSpec(_: Option[PriceType], _: Int))
  }
